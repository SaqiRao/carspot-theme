...
if (!class_exists('carspot_search_ad_color_family')) {
    ...
    class carspot_search_ad_color_family extends WP_Widget
    {
        ...
        function __construct()
        {
            ...
            parent::__construct(false, esc_html__('Carspot:Ad Color Family', 'carspot'), $widget_ops);
        }
        ...
        public function widget($args, $instance)
        {
            ...
            <div class="panel panel-default" id="red-color-family">
                ...
                <!-- Content -->
                <form method="get"
                      action="<?php echo esc_url(get_the_permalink($carspot_theme['sb_search_page'])); ?>#red-color-family">
                    <?php
                    $ad_colors = carspot_get_cats('ad_colors', 0);
                    if (is_array($ad_colors) && count($ad_colors) > 0) {
                        $field_name = 'color_family';
                        $field_name = apply_filters('carspot_search_option_name', $field_name);
                        ?>
                        <div id="ad-color" class="panel-collapse collapse <?php echo esc_attr($expand); ?>"
                                        ...
                                        foreach ($ad_colors as $ad_color) {
                                            ...
                                            <li>
                                                <input type="<?php do_action('carsport_search_option_type'); ?>"
                                                       id="ad_color-<?php echo esc_attr($ad_color->term_id); ?>"
                                                       value="<?php echo esc_attr($ad_color->name); ?>" <?php
