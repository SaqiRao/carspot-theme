...
// Register metaboxes for Products
add_action('add_meta_boxes', 'sb_meta_box_ad_inspection');

function sb_meta_box_ad_inspection() {
    add_meta_box('sb_thmemes_ad_inspection_carspot_metaboxes', __('Inspection ', 'redux-framework'), 'sb_render_meta_ad_inspection', 'inspection', 'normal', 'high');
}

function sb_render_meta_ad_inspection($post) {
    wp_nonce_field('sb_ad_meta_box_nonce_ad_post', 'meta_box_nonce_ad_post');?>
  
    <div class="margin_top">
       
        <select name="_carspot_inspection_status" style="width:100%; height:40px;">
            <option value="0" <?php if (get_post_meta($post->ID, "_carspot_inspection_status", true) == '0') echo 'selected'; ?>>
                <?php echo esc_html__('Pending', 'redux-framework'); ?>
            </option>
             <option value="1" <?php if (get_post_meta($post->ID, "_carspot_inspection_status", true) == '1') echo 'selected'; ?>>
                <?php echo esc_html__('Accepted', 'redux-framework'); ?>
            </option>
             <option value="2" <?php if (get_post_meta($post->ID, "_carspot_inspection_status", true) == '2') echo 'selected'; ?>>
                <?php echo esc_html__('Rejected', 'redux-framework'); ?>
            </option>
...
// if our nonce isn't there, or we can't verify it, bail
    ...
    if (isset($_POST['_carspot_inspection_status'])) {

         update_post_meta($post_id, '_carspot_inspection_status', $_POST['_carspot_inspection_status']);
         $status = get_post_meta($post->ID, "_carspot_inspection_status", true);

         if(isset($status) && $status == 1)
         {

         carspot_get_notify_on_inspection_approval($post);

         }
         elseif(isset($status) && $status == 2)
         {
          carspot_get_notify_on_inspection_reject($post);
         }
