...
/* **** wpml *** */
add_action('init', 'cs_switch_lang_api_callback');
if (!function_exists('cs_switch_lang_api_callback')) {

    function cs_switch_lang_api_callback()
    {
        global $sitepress, $carspotAPI;
        $opt_name = "carspotAPI";
        $carspot_api = get_option($opt_name);
        $lang = carspotAPI_getSpecific_headerVal('Carspot-Lang-Locale');
        //$lang = isset($lang) && $lang != '' ? $lang : 'en';
        if (class_exists('SitePress') && function_exists('icl_object_id') && !is_admin() && $lang != '') {
